...
grp_table_name = os.getenv("GRP_TABLE_NAME")
...
ta_table_name = os.getenv("TA_TABLE_NAME")
ta_index = os.getenv("TA_INDEX")

# Process the group table and put everything in a pandas df
df = process_grp_table(grp_table_name, region, filter_attribute, retained_attribute_list, 
                       component_list,pra_components_list)
# Add TA index to the df
df[ta_index] = df.index + 1

# Write the df to TA dynamodb table

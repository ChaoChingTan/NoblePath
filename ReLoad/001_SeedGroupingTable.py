## Script to upload Grouping info in xlsx to dynamoDB table
## Reads environment variables from .env file to determine the environment and parameters to use
## Based on environment, find the parameters on AWS SSM
...
group_table = get_parameter(f'/{app}/{env}/grp_table')
...
write_to_dynamodb(df, group_table, region_name, row_attribute=row_attribute)
